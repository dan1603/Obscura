...
class ShimmerFrameLayout @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyle: Int = 0) :
    ...
    val maskBitmap: Bitmap?
        get() {
            ...
            val gradient: Shader
            when (mMask.shape) {
                ShimmerFrameLayout.MaskShape.LINEAR -> {
                    val x1: Int
                    val y1: Int
                    val x2: Int
                    val y2: Int
                ...
                ShimmerFrameLayout.MaskShape.RADIAL -> {
                    val x = width / 2
                    val y = height / 2
    ...
    private class Mask {
        ...
        val gradientColors: IntArray
            get() {
                when (shape) {
                    ShimmerFrameLayout.MaskShape.LINEAR -> return intArrayOf(
                        Color.TRANSPARENT,
                        Color.BLACK,
                        Color.BLACK,
                        Color.TRANSPARENT
                    )
                    ShimmerFrameLayout.MaskShape.RADIAL -> return intArrayOf(
                        Color.BLACK,
                        Color.BLACK,
                        Color.TRANSPARENT
                    )
    ...
    private fun tryCreateRenderBitmap(): Bitmap? {
        ...
        try {
            ...
            Timber.d(logMessageStringBuilder.toString())
        }
